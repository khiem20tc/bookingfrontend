...
function UserList(props) {
  ...
  const setHistory = async (event, address) => {
    ...
    await axios
      .put(`http://localhost:5000/user/setHistory/${address}`, history__, {
        headers: {
          Authorization: `${token}`,
        },
      })
            ...
            return (
              ...
              <tr>
                ...
                <td key={index}>
                  <div>
                    <input
                      placeholder="setHistory"
                      type="text"
                      onChange={(event) => setHistory_(event.target.value)}
                    />
                  </div>
                  <form
                    onSubmit={(event) => {
                      setHistory(event, item.address);
                    }}
                  >
